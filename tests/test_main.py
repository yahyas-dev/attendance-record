...
def test_recreate_soft_deleted_attendance():
    ...
    create_response = client.post("/attendance", json=payload, headers=headers)
    assert create_response.status_code == 201
    ...
    recreate_response = client.post("/attendance", json={**payload, "notes": "Recreated after delete"}, headers=headers)
